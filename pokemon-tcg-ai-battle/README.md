...
3. デッキ案を実際のエンジンで自己対戦させて比較し（
  ...
  `obs.logs`のイベントログレベルで精査したところ、相手がKadabra→Alakazamへの
   ...
   [`STRATEGY_REPORT.md`](STRATEGY_REPORT.md) 5.13節参照）。その3日後、実戦の勝率が
   61.8%から32.2%まで下落したため、PR自体の副作用を疑って全カードプールで発火範囲を
   検証（該当カードは1267種中3種のみ）、さらに敗北リプレイ3件を追加解析。結果、
   下落原因は当該PRとは無関係で、**ミル（デッキ切れ）戦略**（Great Tuskの
   「Land Collapse」——自分のデッキ残数を全く監視していない）と、**手札から選んだ
   カードを捨てて分だけダメージが伸びる技**（Team Rocket's Honchkrowの
   「Rocket Feathers」——既存の手札スケーリング検知の文言パターンに一致しない）
  ...
  「Rapid-Fire Combo」等、カードプール全体で4種）を計算できていないことが判明し、
   ...
   [`STRATEGY_REPORT.md`](STRATEGY_REPORT.md) 5.17節参照）。デッキではなくロジック側の
   構造的な限界（0手先読み）に狙いを変え、`opponent_lethal_threat_damage()`（手札
   スケーリング技のみを見る狭いチェック）を、`attack_score()`が自分の技評価に使う
   推定ロジック一式（固定ダメージ・トラッシュ依存ボーナス・コインフリップ期待値・
   手札スケーリング・弱点/抵抗）をそのまま相手の攻撃にも適用する
   `opponent_best_lethal_damage()`に一般化した——カードごとの個別パッチではなく、
   既存の推定ロジックをattacker/defenderを明示的に渡せる形に切り出して両方向で
