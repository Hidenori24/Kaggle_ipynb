...
今後の検討候補（この時点では未実装。5.9節で実際に着手した結果を記録）:
...
### 5.9 一般化した`prize_value`（無印ex=2）は採用、Boss's Ordersは検証の末に不採用

ユーザーから改めて「もっと深く考察してみてください」との指示を受け、5.8節末の2つの
検討候補（Boss's Orders採用、プライズ価値メカニクス自体の深掘り）と、将棋の「詰み」に
類する勝利優先ロジックの有無について、実装前に追加調査を行った。

**無印`ex`も一般化して2プライズと確定**（採用）: 自分のデッキに無印`ex`は存在しないが、
Pikachu ex（無印ex）を組み込んだ独立の最小デッキで検証したところ、撃破時のプライズ
減少は**例外なく2**（48/48件）——実際のポケモンカードの「ex=2プライズ」ルールと一致し、
本エンジンの`megaEx`=3ルールとは別物であることが確認できた。`prize_value(card)`を
「megaExなら3、exなら2、それ以外は1」に一般化。自分のデッキの挙動は変化しない
（無印exを含まないため）が、相手の盤面を評価する際の土台として必要な修正であり、
テスト追加のみで安全に取り込めるため**採用**。

**攻撃対象の選択自体は不可能と確定**（実装見送り）: 攻撃選択（`OPT_ATTACK`）のoptionを
15戦・530件サンプリングしたところ、全件が`{type, attackId}`のみで対象を指定する
フィールドは一切存在しなかった。ベンチに追加ダメージを与える技（`ATTACK_DB`に167件、
"bench"を含むテキストあり）も、実際にOgerpon exの「トーレンシャルポンプ」を追跡した
ところ、対象選択のselectは一切現れずエンジン内部で自動的に対象が決まっていた。つまり、
自分の攻撃で相手の`megaEx`/`ex`を狙う手段は存在しない——プライズを意識した攻めの
唯一の手段は、後述のBoss's Orders等の「強制入れ替え」効果であって、攻撃対象選択では
ない、という結論が確定した。

**Boss's Orders + エネルギー削減は実装・実測の末に不採用**: `bench_snipe_score`/
`best_opponent_bench_snipe_value`（`prize_value`と「今すぐ倒せるか」を組み合わせた
狙撃スコア）を実装し、Boss's Orders×4・基本エネルギー24→20のデッキ（v4候補）で検証した。
実装自体は正しく動作することを確認済み——150戦中99件の「複数候補から選ぶ」場面全てで、
スコアが最大の候補が実際に選ばれていた（新規ロジックのバグではないことを直接確認）。

しかし現行v3デッキとの直接対決A/Bテストでは、2回の独立した600戦バッチがいずれも
明確な悪化を示した：**45.7%**（6バッチ中5つが不利）、**42.7%**（6バッチ中6つ全てが
不利）——プールして44.2%（1,200戦）。他の非エネルギーカードは既に全て4枚上限に達して
おり、Boss's Ordersを入れるための4枚の空きは基本エネルギーを削る以外になかったため、
「エネルギー削減が主因か、Boss's Orders自体が問題か」を切り分ける診断として、
削減幅を半分にした軽量版（Boss's Orders×2・エネルギー24→22のみ）も検証した。
こちらは600戦を2回実施していずれも51.0%（プールして51.0%/1,200戦、バッチ有利率は
まちまち）——v3採用時（51〜53%に3回収束）やプライズ価値退却修正時（3回とも明確に
50%超）と比べて、明確な改善とは言えない、ノイズとほぼ区別できない水準だった。

この結果から、エネルギー削減幅がAI版・軽量版どちらでも主要な悪化要因であり、
Boss's Orders自体の狙撃ロジックが誤っているわけではないと判断できる。一方で、
このデッキの他の非エネルギーカード（Cyrano・Powerglass等）は5.6節までの検証で
すでに「削ると明確に悪化する」と判明済みであり、エネルギー以外に空きを作る手段が
実質的にない。つまり60枚中のこの核（v3）は、この自己対戦環境に対してすでにかなり
最適化されており、Boss's Ordersを追加できる「無料の空き」が存在しない可能性が高い。

**不採用**。実装した`bench_snipe_score`等のロジックはBoss's Ordersが実際にデッキに
入っていない限り一切発火しない死んだコードになるため（Powerglassのツールダメージ
問題を見送った際と同じ「デッキに存在しないカードのためのコードは書かない」方針に従い）
main.py本体からは削除し、`prize_value`の一般化のみを本体に残した。この節は5.6節・
6章と同じ「正直に記録する」方針に基づく、実装・実測まで踏み込んだ上での不採用の記録
であり、将来Boss's Orders以外の切り口（例えば他の非エネルギーカードを1種類だけ減枠する、
より小さいエネルギー削減幅を試す等）を試す際の参考として残す。

**「詰み」に相当するロジックの現状**（ユーザーからの質問への回答、実装は見送り）:
`attack_is_lethal()`は既に「今のターンで倒せるなら常に最優先」を実装しており、将棋の
「一手で終わらせられるなら終わらせる」に相当する判断は存在する。一方で次の3点は
明確に未実装と確認した: (1) 装備中のツールカードによるダメージ補正——
`attack_score`/`attack_is_lethal`は`active.get("tools")`を一切参照しない。ただし
現行デッキの唯一のツールであるPowerglassは実測（後述）でダメージに無関係と確定した
ため実害なし。(2) 1手先の相手の反撃を読んだ「犠牲を許容してでも攻める」判断——
既存タスクリスト#17・7章の「簡易先読み」として以前から認識済みだが未実装のまま。
(3) 複数ターンにわたる「Nターン後に必ず勝てる」探索（真の詰み探索）——今回は
実装しないより大きな別課題として記録。

**Powerglassの実効果を実測で確認**: (1)の検証のため、Powerglass（cardId 1163、
現行デッキの唯一のツール）の実際の効果を確認した。テキストは「自分のターン終了時、
アクティブの場のポケモンに捨て札から基本エネルギーを1枚再装着できる」——ダメージには
一切関係しない、エネルギー経済系のツールだった。25戦の実測でも、Powerglass装備前後で
攻撃ダメージの分布に統計的な差は見られなかった（未装備116件平均20.34、装備217件
平均19.59——コインフリップのブレの範囲内）。ダメージを上げるツール自体はカードプールに
存在する（Maximum Belt 1158等）が、いずれも現行デッキには入っておらず、Maximum Beltは
既にACE SPECとしての採用を試して不採用（5,400戦、6章参照）としている。以上より、
...
### 5.11 コインフリップ技の期待値未反映を修正——ただし`attack_is_lethal`側は
A/Bで裏目に出て一部差し戻し

引き続き改善点を洗い出す中で、`attack_score()`/`attack_is_lethal()`/
`cheap_reliable_damage()`がどれも`ATTACK_DB`の確定ダメージ値だけを見ており、
「コインフリップで表が出れば追加ダメージ」という条件付きボーナスを一切考慮して
いないことに気づいた。しかもこれは他デッキの話ではなく、**自分のRioluの技
「Quick Attack」自体がこのパターン**（`"Flip a coin. If heads, this attack does
20 more damage."`——確定10、期待値20）だったため、自己対戦でA/Bテスト可能な
珍しいケースだった。

`_coin_flip_bonus(text)`を追加し、当初は`attack_score()`に期待値（半分の
ボーナス）を、`attack_is_lethal()`に最大値（満額のボーナス）を反映する実装で
検証した。ロジックとしては「表が出れば倒せるなら、他にやることがない以上
賭ける価値がある」という理屈だったが、実際にA/Bテストしたところ3回の独立
600戦バッチ（47.3%・50.0%・49.2%、プールして48.8%/1,800戦）が僅かに劣勢で
収束し、Boss's Ordersのような明確な悪化ではないものの、明確な改善とも言えない
結果だった。

`attack_score`側の期待値反映と`attack_is_lethal`側の「賭けに出る」再分類の
どちらが原因かを切り分けるため、`attack_is_lethal`だけを元の確定ダメージのみに
戻した「EV限定版」を用意し、同じ基準に対して再検証した。結果は3回の独立600戦
バッチで51.3%・51.3%・54.5%（プールして52.4%/1,800戦、17/18バッチが有利）と
明確に好転——さらに実際に本番コードへ反映した後の追加確認2回（51.2%・48.8%）を
含めた合計5回・3,000戦でも51.4%と、一貫してプラス側に収まった。

つまり、「50%の確率でしか当たらない一撃を、確殺と同格の最優先タイミング
（tier 11）にまで格上げする」判断自体が損だった、という結論になる——外れた
場合に失う「確実な地ならし（エネルギー付け・進化）」の期待損失が、賭けに勝った
時の得点を上回っていたとみられる。**`attack_score()`の期待値反映のみ採用し、
`attack_is_lethal()`は確定ダメージのみに差し戻した。**

この節は、「理屈の上では妥当に見える拡張でも、A/Bで裏目に出た部分だけを
切り分けて差し戻す」という、6章で以前から実践してきた方針を、今回は
...
### 5.9 Generalizing `prize_value` (ex = 2) shipped; Boss's Orders was implemented, measured, and rejected

Asked to think even more deeply, we followed up on both of Section 5.8's deferred items plus a
separate question about whether the agent has any "checkmate"-style win-finding logic, before
writing any more code.

**Plain `ex` generalizes cleanly to 2 prizes — adopted.** Our own deck has no plain-`ex` Pokemon, so
this was untested until now. Built an isolated minimal deck around Pikachu ex (a plain `ex`) and
measured its KO events the same way as the original megaEx probe: **exactly 2 prizes, 48/48 events,
zero exceptions** — matching the real paper TCG's "ex = 2 prizes" rule, distinct from this engine's
`megaEx` = 3 rule. Generalized `prize_value(card)` to "megaEx → 3, ex → 2, else → 1." Behavior
against our own deck is unchanged (we run no plain-ex Pokemon), but it's the correct evidence-backed
form and the foundation any opponent-board evaluation (e.g. a gust target) needs. Safe, tested,
**adopted**.

**Attack targeting is definitively impossible to make prize-aware — confirmed, not implemented.**
Sampled 530 `OPT_ATTACK` options across 15 games: every single one carried only `{type, attackId}`,
never a target field. Even attacks that deal bench-splash damage (167 attacks mention "bench" in
`ATTACK_DB`; directly traced Ogerpon ex's Torrential Pump) resolve their target internally in the
engine with no select ever offered. So there is no way to make our own attacks preferentially hit an
opponent's `megaEx`/`ex` — the only lever for offensive prize-awareness is a forced switch-in effect
(Boss's Orders), not attack selection. This closes the question definitively rather than leaving it
open.

**Boss's Orders + trimming Energy: implemented, verified correct, and rejected after real A/B
testing.** Implemented `bench_snipe_score`/`best_opponent_bench_snipe_value` (prize_value combined
with a "killable this turn" check) and built a v4 candidate deck (Boss's Orders x4, Basic Energy
24→20 — the only way to fit it in, since every other non-Energy card was already at its 4-copy
legal max). Verified the targeting logic itself was correct before judging the deck: across 150
games, all 99 sampled multi-candidate opponent-bench-target decisions picked the actual
highest-scoring candidate (0 mismatches) — the implementation does what it's supposed to.

Head-to-head against the shipped v3 baseline, two independent 600-game runs both showed a clear,
consistent loss: **45.7%** (5/6 batches unfavorable) and **42.7%** (6/6 batches unfavorable) — pooled
44.2% over 1,200 games. To isolate whether the Energy trim or Boss's Orders itself was the driver, a
lighter variant (Boss's Orders x2, Energy trimmed only 24→22) was tested next: two more 600-game
runs both landed at 51.0% (pooled 51.0% over 1,200 games, but with mixed batch favorability, unlike
the tight, consistently-favorable bands that justified shipping v3 or the retreat fix) — indistinguishable
from noise by this project's established bar, not a clear win.

This points to the Energy trim itself (not the targeting logic) as the main driver of the
regression — and by elimination, there may simply be no free 4-card slot in this specific 60-card
core for Boss's Orders: every other non-Energy card was already proven load-bearing in this
project's own history (Section 5.6 recorded that cutting Cyrano or Powerglass for Farfetch'd both
measured as clear regressions), leaving Energy as the only cuttable resource despite the cost.
**Rejected.** Following the same principle used to set aside the Powerglass/Tool-damage question
below (no code for a card that isn't in the deck), the `bench_snipe_score` logic — inert without
Boss's Orders actually in the deck — was removed from `main.py`; only the `prize_value` generalization
was kept. Recorded here in the same honest-failure-recording spirit as Section 5.6/6, in case a
different angle (cutting a different single card instead of Energy, or a smaller Energy trim) is
worth trying later.

**Current state of "checkmate"-style win-finding logic** (a direct answer to a user question, not
implemented this round): `attack_is_lethal()` already always takes any single-attack KO available
this turn at the top priority tier — the closest existing equivalent to "finish it when you can."
Three real gaps were identified: (1) `attack_score`/`attack_is_lethal` never read
`active.get("tools")`, so a damage-boosting Tool would be invisible to lethal detection — investigated
concretely below and found to cause zero live impact today; (2) no 1-ply opponent-counterattack
lookahead for "commit to a losing trade instead of retreating" — already a known, separately-tracked
item (task #17 / Section 7); (3) no true multi-turn forced-win search — a much larger, separate
effort, not attempted here.

**Powerglass's real effect, measured.** Investigating (1) concretely: Powerglass (cardId 1163, the
only Tool in our current deck) was the natural suspect. Its actual text: "at the end of your turn,
you may attach a Basic Energy card from your discard pile to the Pokemon this card is attached to"
— a pure energy-economy Tool, not a damage boost. Confirmed by direct measurement over 25 games: 116
sampled attacks without Powerglass attached vs. 217 with it showed statistically identical damage
distributions (means 20.34 vs. 19.59 — within the noise of the attack's own coin-flip mechanic).
Real damage-boosting Tools do exist in the card pool (Maximum Belt 1158 among others), but Maximum
Belt was already tried as a deck addition and rejected after 5,400 games (Section 6), and none of
...
### 5.11 Coin-flip attack expected value fixed -- but the `attack_is_lethal` half of it backfired in A/B and was partially reverted

Continuing the search for improvements, `attack_score()`/`attack_is_lethal()`/`cheap_reliable_damage()`
were all found to read only an attack's flat, guaranteed `damage` field, completely ignoring "flip a
coin, +N damage on heads" bonuses. Unlike most gaps found this session, this one directly affects our
own deck: **Riolu's own "Quick Attack"** has exactly this pattern ("Flip a coin. If heads, this attack
does 20 more damage." — flat 10, expected value 20), making it a rare case actually testable via
self-play A/B.

Added `_coin_flip_bonus(text)` and initially wired it into both functions: the expected value (half
the stated bonus) into `attack_score()`, and the full stated bonus into `attack_is_lethal()` — on the
reasoning that "if a coin flip could end the game right now and there's nothing better to do, it's
worth the gamble." Tested as a whole first: three independent 600-game batches came back at 47.3%,
50.0%, and 49.2% (pooled 48.8%/1,800 games) — not the clear regression Boss's Orders showed, but not a
clear win either, and consistently *not quite* clearing 50%.

To isolate which half was responsible, a diagnostic variant reverted only `attack_is_lethal` back to
flat-damage-only while keeping `attack_score`'s expected-value fix. Re-tested against the same
baseline: three independent 600-game runs came back at 51.3%, 51.3%, and 54.5% (pooled 52.4%/1,800
games, 17/18 individual batches favorable) — a clear improvement. Two further confirmation runs against
the actual shipped file (after a small refactor simplifying `_coin_flip_bonus`'s return signature)
landed at 51.2% and 48.8%, for a combined total across all five runs of 51.4% over 3,000 games — not
every single batch favorable, but a consistent, clearly-positive pooled signal, matching the same
"most batches favorable, strong pooled majority" bar used to ship deck v3.

The takeaway: reclassifying a 50/50 shot as top-priority-tier "guaranteed lethal" apparently costs more
in forgone certain setup (energy attachment, evolving) on the misses than it recovers from the
successful gambles. **`attack_score()`'s expected-value correction was kept; `attack_is_lethal()` was
reverted to flat-damage-only.**
