...
### 5.13 5.12節の判断を反転——`opponent_lethal_threat_damage()`を進化先1段まで拡張して採用

5.12節の提出後もラダーでの実戦結果を追跡した結果（submission 54778318、
2026-07-18〜07-20の3日間で21勝13敗・61.8%——前回集計の45.8%から明確に
改善）、全体としては上向きだったが、依然として「turn3・HP70・ベンチ0」
のような早期敗北や、HP100・HP280のまま負ける不自然なケースが散見された。
5.12節の「1件だけでは根拠不十分」という判断を見直し、`opponent_lethal_
threat_damage()`を実際に拡張することにした。

実装：`CARD_DB`全体から「どのカードがどのカードから進化するか」の逆引き
テーブル（`EVOLUTIONS_BY_BASE_NAME`、`OWN_DECK_EVOLUTION_BASES`と同じ
「起動時にCARD_DBから動的に構築する」方針を踏襲）を新設し、相手のアクティブ
の**現在の技**だけでなく、**そこから1段進化した先のカードの技**も同じ
手札スケーリング検知にかける。エネルギーは進化しても引き継がれる（
episode #86220242で直接確認済み）ため、現在付いているエネルギー数を
そのまま使い回して判定できる。

検証は5.10節で確立した基準（(1) 実リプレイで直接メカニズムを確認済み——
まさに5.12節のKadabra→Alakazam進化+Powerful Handの事例、(2) 新ロジックを
実際のobsに再適用し、発火条件が正しいことを確認、(3) 自己対戦で無害である
ことを確認）に従った。(2)は5.12節で解析した状況（Kadabraがアクティブで
Psychicエネルギー1枚付き、相手の手札26枚）をそのまま`obs`として再構成し、
`opponent_lethal_threat_damage()`が520（≥HP340）を返し`active_in_danger()`
がTrueになることを確認。(3)は自分のRiolu→Mega Lucario ex進化ラインには
手札スケーリング技が存在しないため理論上ノーオペのはずで、実測でも300戦・
47.7%（143W-157W）と、このプロジェクトで校正済みのノーオペ由来ノイズ帯
（43〜57%）に収まることを確認した。**採用**。

### 5.14 PR #29後の勝率下落（61.8%→32.2%）を実リプレイで検証——原因はPR #29ではなく、
2つの全く新しい盲点だった

PR #29適用後の3日間（submission 54870466、59試合）で勝率が61.8%から32.2%
（19勝40敗）まで下落した。PR #29自体の副作用を疑い、まずロジックの発火範囲を
全カードプール（1267種）で検証したところ、手札スケーリング技を持つカードは
**3種のみ**（Alakazam・Chandelure・Mega Froslass ex）、進化先読みが反応しうる
進化元名も3つ（Kadabra・Snorunt・Lampent）に限られることを確認——これほど狭い
発火範囲では、多様な対戦相手59試合全体にわたる広範な勝率低下を単独で説明できない。
また`kaggle_watch.py`の敗因ログにクラッシュ/不正選択によるものは1件もなく、PR #29
がバグで対戦を落としていた形跡もなかった。

そこで「HP満タンのまま負ける」敗北リプレイを追加で3件取得し、`obs.logs`で直接
原因を追跡したところ、**PR #29とは無関係な、2つの全く新しい盲点**が見つかった：

1. **ミル（デッキ切れ）戦略**（episode #87498120）: Great Tuskの「Land Collapse」
  （相手のデッキの一番上を1枚トラッシュ、Ancientサポーターを使ったターンは
   さらに3枚）を毎ターン受け続け、デッキが枚数不足になって敗北。HPは一度も
   削られていない。現在のエージェントは自分のデッキ残数を一切監視していない。
2. **手札の任意枚数を選んで捨てる型の技**（episode #87536235）: Team Rocket's
   Honchkrowの「Rocket Feathers」（手札から「Team Rocket」を含むSupporterを
   任意枚数捨て、捨てた枚数×60ダメージ）で340ダメージ（6枚分360）の一撃必殺。
   既存の`_ATTACKER_HAND_DAMAGE_RE`は「for each card in your hand」という
   固定手札枚数依存の文言にしか一致せず、「discard ... for each card you
   discarded」という自己選択式の文言は検知対象外。
   （なお、もう1件・episode #87577878はDragapult exの通常攻撃＋毎ターンの
   減少ダメージによる通常のKOで、自分視点のHP停滞は5.10節で既知のkaggle_watch.py
   観測遅延アーティファクトそのものだった——新しい問題ではない。）

Rocket Feathers系（「捨てた枚数×固定ダメージ」だが捨てる枚数は自己選択かつ
`obs`から見える`handCount`はカードの内訳（何枚が対象カードか）を含まない）は、
カードプール全体で調べると同系統の技が13種見つかった。うち10種は**自分の
エネルギーを捨てる**タイプ（対象がactive/benchのどちらの装備エネルギーかは
技文次第だが、いずれにせよ相手の場のエネルギー枚数という直接観測可能な値で
安全に上限を見積もれる）で、残り3種（Rocket Feathers・Mega Clefable exの
「Shooting Moons」・Mawileの「Double Eater」）は**手札の中の特定カードを
選んで捨てる**タイプで、`handCount`しか見えない以上、上限を「手札枚数全部が
対象カード」という最悪ケースで見積もるしかなく、実際にはそのデッキの一部
（本命コンボでなければ数枚程度）しか対象カードを持たないケースでも大きく
過大評価し、不要な撤退を誘発するリスクが高い。

以上を踏まえ、今回は**どちらも実装を見送り、調査結果の記録のみ**とした——
5.12節と同じ「1件の実例だけでは、安全に境界を引ける対策を組む根拠として
不十分」という基準に従っている。勝率下落そのものについては、PR #29の
...
### 5.13 Reversing Section 5.12's call: extended `opponent_lethal_threat_damage()` one evolution stage ahead, and shipped it

Real ladder results kept coming in after Section 5.12's submission (submission 54778318, three days
from 2026-07-18 to 2026-07-20: 21 wins, 13 losses -- 61.8%, a clear step up from the 45.8% tallied
before that round). Encouraging overall, but the loss list still showed the same shapes that motivated
5.12 in the first place: an implausibly early "turn 3, active HP 70, empty bench" loss, and a couple of
losses at HP 100/HP 280 rather than 0. Section 5.12's "one replay isn't enough evidence" call was
revisited, and `opponent_lethal_threat_damage()` was actually extended this round.

Implementation: a new reverse-lookup table built once from the full `CARD_DB` at import time
(`EVOLUTIONS_BY_BASE_NAME`, following the same "compute it dynamically from CARD_DB rather than
hardcode a card name" discipline as `OWN_DECK_EVOLUTION_BASES`) maps a Pokemon's name to every card
that lists it as `evolvesFrom`. The hand-scaling check now runs against the opponent active's *current*
card **and** against whatever it could evolve into next. Energy carries over through evolution in this
engine (directly confirmed in episode #86220242), so the same currently-attached Energy count can be
reused to gate the evolution's attacks too.

Validated against the same bar established in Section 5.10: (1) the mechanism is already verified
directly against real match data -- this is exactly the Kadabra-into-Alakazam-plus-Powerful-Hand
sequence traced in Section 5.12; (2) re-applying the new logic to that same real board state (Kadabra
active, 1 Psychic Energy attached, opponent holding 26 cards) now returns 520 damage (>= our 340 HP),
correctly flipping `active_in_danger()` to `True` *before* the opponent gets a chance to evolve and
fire; (3) confirmed harmless in self-play, since our own Riolu -> Mega Lucario ex evolution line has no
hand-scaling attack anywhere in it -- 300 self-play games came back 143W-157W (47.7%), squarely inside
this project's calibrated no-op noise band (43-57%). **Adopted.**

### 5.14 A post-#29 win-rate dip (61.8% -> 32.2%) investigated via real replays -- not PR #29's fault, but two entirely new blind spots

Three days after PR #29 shipped (submission 54870466, 59 games), the win rate dropped from 61.8% to
32.2% (19W-40L). Before chasing new fixes, PR #29 itself was audited as the suspect: scanning the
*entire* 1267-card pool showed only 3 cards anywhere have a hand-scaling attack (Alakazam, Chandelure,
Mega Froslass ex), and only 3 pre-evolution names could ever trip the new evolution-lookahead check
(Kadabra, Snorunt, Lampent) -- far too narrow a trigger surface to explain a broad-based drop across 59
games against varied opponents. `kaggle_watch.py`'s loss log also showed zero crash/invalid-action
forfeits in that span, ruling out a code-level bug from PR #29.

To find the real cause, three more "died at full HP" loss replays were pulled and traced through
`obs.logs` directly, turning up **two entirely new blind spots, both unrelated to PR #29**:

1. **Deck-out (mill) loss** (episode #87498120): Great Tusk's "Land Collapse" ("Discard the top card
   of your opponent's deck. If you played an Ancient Supporter card from your hand during this turn,
   discard 3 more cards in this way.") ground our deck down turn by turn with zero HP damage taken --
   the agent has no awareness of its own remaining deck size at all.
2. **Self-chosen-discard scaling attack** (episode #87536235): Team Rocket's Honchkrow's "Rocket
   Feathers" ("You may discard any number of Supporter cards that have 'Team Rocket' in their name
   from your hand, and this attack does 60 damage for each card you discarded in this way.") one-shot
   our 340-HP active for 360 (6 cards discarded). The existing `_ATTACKER_HAND_DAMAGE_RE` only matches
   the fixed "for each card in your hand" phrasing -- this self-chosen-discard wording doesn't match it
   at all.
   (A third replay, episode #87577878, turned out to be an ordinary flat-damage KO with no new
   mechanism -- the "HP still shown at loss" was just the already-documented Section 5.10 stale-
   self-view logging artifact, nothing new.)

A full-pool audit found 13 cards with this general "discard N cards, deal M damage per card discarded"
shape. 10 of them discard the attacker's *own attached Energy* (from itself or its bench) -- a resource
we can observe exactly and bound safely, the same way the existing checks do. The other 3 (Rocket
Feathers, Mega Clefable ex's "Shooting Moons", Mawile's "Double Eater") discard *chosen cards from
hand* -- since `obs` only exposes `handCount`, not hand contents, any defense here would have to assume
a worst case of "the entire hand is eligible," which would badly overestimate the true threat (and
trigger unnecessary retreats) against the far more common case of a deck running the card as a 1-of
rather than a dedicated combo.

Given only one confirmed real instance of each new pattern so far, and the hand-discard variant's
unbounded false-positive risk without visibility into hand contents, **neither was implemented this
