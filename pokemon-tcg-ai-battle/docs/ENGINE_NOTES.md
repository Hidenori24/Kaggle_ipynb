...
- `result`: `-1`=継続中, `0`=プレイヤー0勝利, `1`=プレイヤー1勝利, `2`=引き分け。
  ⚠️ **試合終了後、最後に記録された`observation`内の`current.result`が`-1`（継続中）の
  ままになっている場合がある**（実測: `env.run()`の戻り値の最終ステップで両プレイヤーとも
  `-1`だったが、そのステップ自身の`status`は`"DONE"`、`reward`は`1`/`-1`と正しく入っていた）。
  `current`はネイティブエンジンがその時点の盤面として返したスナップショットで、`reward`/
  `status`はそれとは別に`kaggle_environments`側が試合終了後に確定させるフィールドらしく、
  タイミングがずれることがある。**勝敗判定には`current.result`ではなく、そのステップ自身の
  `status`/`reward`を使うこと**（`tools/evaluate.py`等、本リポジトリの既存コードは元々
  そうしている）。
- 自分の手札は常にフルオブジェクト、相手の手札は `null`（`handCount` のみ判明）。
- ポケモンオブジェクトの `hp` は**現在HP**（`maxHp` が最大HP）。
- ⚠️ **盤面スロット0（先手側）に実測で有意な有利がある。** 同一エージェント同士
  （`submission/main.py`を2つの独立モジュールとしてロード）を戦わせて計測したところ、
  スロット0側の勝率は 1,882W-1,718L/3,600戦 = **52.28%**（厳密二項検定 p=0.0066）。
  交替を無効化して常に同じ側に固定した対照実行では 640W-560L/1,200戦 = 53.33%
  （p=0.023）と、**中身が完全に同一のエージェント同士なのに明確に偏る**。
  したがって**自己対戦でA/Bを取る際は、必ず1戦ごとに盤面side を交替させること**
  （本リポジトリのA/Bスクリプトは `g % 2` で交替しており、同条件・交替ありでは
  50.33%/2,400戦・p=0.76と正しく50%に収束することを確認済み）。この交替を
  「簡潔化」して外すと全A/B結果が約2〜3ポイント上振れし、効果ゼロの変更を
  堂々とした改善に見せてしまう。詳細は`STRATEGY_REPORT.md` 5.19節。
...
- ~~攻撃のダメージ計算に「弱点2倍・抵抗-30」等の実際の補正ルールが適用されるかは未検証~~
  **検証済み・実装済み**。既知の固定ダメージ技（Riolu の Quick Attack、Mega Lucario ex の
  Aura Jab）を、弱点=闘/抵抗=闘の相手カード（Team Rocket's Kangaskhan ex / Iron Crown ex 等、
  `AllCard()`から`weakness`/`resistance`が確認できるカードを対戦相手デッキに組んで）に当てて
  実際のHP変化を追跡するプローブスクリプトで確認: 弱点一致で額面ダメージの**x2**、抵抗一致で
  **-30**（0未満にはならない）。実際のポケモンカードの現行ルールと完全に一致する数値。
  `submission/main.py`の`apply_weakness_resistance()`で`attack_score`/`attack_is_lethal`両方に
  適用済み（実装前は「額面ダメージだけで判定していたため、弱点込みなら確定KOだった一撃を
  『まだ倒せない』と誤判定していた」ケースが実測で1試合あたり平均1.2回程度発生していた）。
